...
mod tests {
    ...
    use crate::services::submission::support::{
        ProblemSupport, SanityCheckInput, SanityCheckProblemSupport, SubmissionGenerationPayload,
        SubmissionTestGenerationPayload, VerificationPayload,
    };
    ...
    fn sanity_check_generates_details() {
        ...
        let details = support
            .generate_submission_details(SubmissionGenerationPayload {
                seed: 10,
                sample_index: None,
            })
            .unwrap();
    ...
    fn sanity_check_generates_input() {
        let support = get_support();
        let details = support
            .generate_submission_test_input(SubmissionTestGenerationPayload {
                seed: 10,
                test_index: 3,
                sample_index: None,
            })
        ...
        let inp = serde_json::from_value::<SanityCheckInput>(details.input).unwrap();
    ...
    fn sanity_check_generates_output() {
        ...
        let details = support
            ...
            .unwrap();
        let inp = serde_json::from_value::<SanityCheckInput>(details.input.clone()).unwrap();
    ...
    fn sanity_check_generates_output_conformity() {
        ...
        let output = SanityCheckProblemSupport::solver(&details.input.to_string()).unwrap();
        let verf = support
            .verify_output(VerificationPayload {
