...
impl GithubAuthenticatorService {
    pub fn exchange_code_for_token(&self, code: &str) -> anyhow::Result<String> {
        ...
        let resp = resp_reqw.json::<HashMap<String, String>>().map_err(|err| {
        ...
        resp.get("access_token")
            ...
            .ok_or_else(|| {
                error!(
                    "No access token in request, full response is: {}",
    ...
    fn get_user_email_from_token(&self, token: &str) -> anyhow::Result<String> {
        ...
        headers.insert(
            ...
            header::HeaderValue::from_str(&format!("token {}", token))
                .map_err(|_| anyhow::Error::msg("No header!"))?,
        );
        ...
        let resp = resp_one.json::<Vec<EmailsResponseItem>>().map_err(|err| {
...
impl<'a, 'r> FromRequest<'a, 'r> for GithubAuthenticatorService {
    ...
    fn from_request(
        req: &'a Request<'r>,
    ) -> request::Outcome<GithubAuthenticatorService, Self::Error> {
        ...
        request::Outcome::Success(GithubAuthenticatorService::new(config.inner().clone()))
    }
}
