...
use crate::models::problem::problem_statement::{NewProblemStatement, ProblemStatement};
...
crud_repo!(
    ProblemStatementCrudRepo,
    DbProblemStatementCrudRepo,
    problem_statements,
    ProblemStatement,
    NewProblemStatement,
    "Problem statement"
...
macro_rules! full_availability {
    ($now: ident) => {
        problems::available_from
            .le($now)
            .or(problems::available_from.is_null())
            .and(
                problems::available_until
                    .ge($now)
                    .or(problems::available_until.is_null()),
            )
    };
}
...
pub trait ProblemRepo {
    ...
    fn find_available_problems(
        &self,
        now: NaiveDateTime,
        td: &ITransaction,
    ) -> anyhow::Result<Vec<Problem>>;

    fn find_next_available_problem(
        &self,
        now: NaiveDateTime,
        td: &ITransaction,
    ) -> anyhow::Result<Problem>;

    fn find_available_problem_by_code(
        &self,
        code_name: &str,
        now: NaiveDateTime,
        td: &ITransaction,
    ...
    fn find_available_problem_by_code_name_populated(
        &self,
        code_name: &str,
        now: NaiveDateTime,
        td: &ITransaction,
    ) -> anyhow::Result<(Problem, ProblemStatement)>;
}
...
impl ProblemRepo for DbProblemRepo {
    ...
    fn find_available_problems(
        ...
        problems::table
            ...
            .filter(problems::is_deleted.eq(false).and(full))
    ...
    fn find_next_available_problem(
        ...
        problems::table
            .select(problems::all_columns)
            .filter(
                problems::is_deleted.eq(false).and(
                    problems::available_from
                        .is_not_null()
                        .and(problems::available_from.ge(now))
                        .and(
                            problems::available_until
                                .is_not_null()
                                .and(problems::available_until.ge(now))
                                .or(problems::available_until.is_null()),
                        ),
                ),
            )
    ...
    fn find_problem_by_code(&self, code_name: &str, td: &ITransaction) -> anyhow::Result<Problem> {
        problems::table
            .filter(problems::code_name.eq(code_name))
            .first::<Problem>(td.get_db_connection())
            .map_err(|_| anyhow::Error::msg("Problem not found!"))
    }
